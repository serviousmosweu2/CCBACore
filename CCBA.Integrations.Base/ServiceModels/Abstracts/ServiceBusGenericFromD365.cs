...
{
    /// <summary>
    /// Developer: Johan Nieuwenhuis, Konrad Stenberg
    /// Dependencies: <see cref="StopwatchService"/>, <see cref="ExtractLegalEntity"/>
    /// </summary>
    public abstract class ServiceBusGenericFromD365<T> : AFunction
    {
        protected Message Message;
        protected MessageReceiver MessageActions;

        private readonly ExtractLegalEntity _extractLegalEntity;
        private readonly ILogger<ServiceBusGenericFromD365<T>> _logger;
        private readonly StopwatchService _stopWatchService;

        protected ServiceBusGenericFromD365(ILogger<ServiceBusGenericFromD365<T>> logger, IConfiguration configuration, ExtractLegalEntity extractLegalEntity, StopwatchService stopWatchService) : base(logger, configuration)
        {
            _logger = logger;
        ...
        protected async Task BaseRun(ExecutionContext context, Message message, Func<Dictionary<string, string>, string> getLegalEntityFromMessage, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                {
                    await Main(JsonConvert.DeserializeObject<T>(value));
                }
        ...
        protected abstract Task Main(T message);
